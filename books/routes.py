...
def api_get_books(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """Get all books"""
...
def api_update_book(book_id: int, book: BookUpdate, db: Session = Depends(get_db)):
    """Update a book"""
...
def api_create_rental(rental: RentalCreate, db: Session = Depends(get_db)):
    """Create a new rental"""
    return create_rental(db, rental)

@router.get("/rentals/", response_model=List[RentalResponse])
def api_get_rentals(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """Get all rentals"""
    return get_rentals(db, skip, limit)

@router.get("/rentals/{rental_id}", response_model=RentalResponse)
def api_get_rental(rental_id: int, db: Session = Depends(get_db)):
    """Get a specific rental"""
    return get_rental(db, rental_id)

@router.get("/users/{user_id}/rentals", response_model=List[RentalResponse])
def api_get_user_rentals(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def api_return_book(rental_id: int, db: Session = Depends(get_db)):
    """Return a rented book"""
    return return_book(db, rental_id)

@router.get("/rentals/overdue", response_model=List[RentalResponse])
def api_get_overdue_rentals(db: Session = Depends(get_db)):
